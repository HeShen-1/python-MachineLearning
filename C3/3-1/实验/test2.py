...
def validate_model(knn, X_test, y_test):
    ...
    # 选择一些测试样本进行可视化
    num_images = 40
    images = X_test.reshape(-1, 8, 8)  # 重塑为8x8的图像
    # 将图像、预测结果和真实标签组合在一起
    images_and_labels = list(zip(images, y_pred, y_test))

    # 创建可视化的画布
    plt.figure(figsize=(10, 10))
    for index, (image, prediction, actual) in enumerate(images_and_labels[:num_images]):
        plt.subplot(5, 8, index + 1)  # a行b列显示n张图像
        plt.imshow(image, cmap=plt.cm.gray_r, interpolation='nearest')     # 显示灰度图像:plt.cm.gray_r\'gray'(黑字白底、白字黑底)
    ...
    sns.heatmap(conf_matrix, annot=True, fmt="d", cmap="Blues", xticklabels=digits.target_names,
